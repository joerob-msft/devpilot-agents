...
try {
    ...
    $duplicateAttributes = Get-ReviewerConstructAttributeFrequency -DeclarationIndex @(
        [pscustomobject]@{ Truncated = $false; Attributes = @("TestCase", "TestCase") },
        [pscustomobject]@{ Truncated = $false; Attributes = @("TestCase", "TestCase") })
    Assert-Replay ([int]@($duplicateAttributes.Attributes)[0].declarations -eq 2) `
        "Attribute frequency must count comparable declarations, not repeated attribute occurrences."
    ...
    $generatedByPath = Get-Constructs -Code @('public class GeneratedType { }') -Path "src/generated/Type.cs"
    Assert-Replay ([bool]@($generatedByPath.Files)[0].generatedCode) `
        "A common generated-code path must be sealed as generated in file evidence."
    $generatedByHeader = Get-Constructs -Code @('// <auto-generated />', 'public class GeneratedType { }')
    Assert-Replay ([bool]@($generatedByHeader.Files)[0].generatedCode) `
        "A standard generated-code header must be sealed as generated in file evidence."
    $ordinarySource = Get-Constructs -Code @('// Tests generated code behavior.', 'public class SourceType { }')
    Assert-Replay (-not [bool]@($ordinarySource.Files)[0].generatedCode) `
        "Ordinary source commentary must not be classified as a generated-file marker."
    ...
    function New-ReconCandidate {
        param(
            ...
            [string]$RemediationAction = "modify", [string]$RemediationTargets = "mi0",
        ...
        return [pscustomobject][ordered]@{
            ...
            changedCodeFix = [pscustomobject][ordered]@{
                action = $RemediationAction; targets = $RemediationTargets
                conventionKey = $ConventionKey; valueSource = $ChangedValueSource
                evidenceFactIds = $ChangedEvidenceFactIds
            }
            existingDebtFollowUp = [pscustomobject][ordered]@{
                status = $DebtStatus; evidenceFactId = $DebtFactId
                selectorKey = $DebtSelectorKey
                scopeKind = $(if ($DebtStatus -ceq "required") { "file" } else { "" })
                scopePath = $DebtScopePath; comparableCount = $ComparableCount
                compliantCount = $CompliantCount; action = $DebtAction
            }
        }
    }
    ...
    $factBackedCandidate = New-ReconCandidate -ChangedValueSource deterministicFact `
        -ChangedEvidenceFactIds $changedFixFactId
    $factBackedAgreement = Resolve-ReviewerRunReconciliation -Manifests @(
        (New-ReconRun -Nonce "fact-a" -Rows @((New-ReconRow)) `
                -Candidates @($factBackedCandidate) -RemediationFacts @($changedFixFact)),
        (New-ReconRun -Nonce "fact-b" -Rows @((New-ReconRow)) `
                -Candidates @($factBackedCandidate) -RemediationFacts @($changedFixFact)))
    ...
    $debtCandidateA = New-ReconCandidate -Path "/src/semantic.cs" -Line 1034 `
        ...
        -DebtSelectorKey TestCase `
        -DebtScopePath "src/semantic.cs" -ComparableCount 38 -CompliantCount 0 `
        -DebtAction recordTrackedFollowUp
    $debtCandidateB = New-ReconCandidate -Path "SRC\SEMANTIC.cs" -Line 1034 `
        -Comment "Track the existing local debt after this change." -RemediationTargets dc0 `
        -ConventionKey RequiredAnnotation -DebtStatus required -DebtFactId $debtEvidenceId `
        -DebtSelectorKey TestCase `
        -DebtScopePath "/src/semantic.cs" -ComparableCount 38 -CompliantCount 0 `
        -DebtAction recordTrackedFollowUp
    $debtAgreement = Resolve-ReviewerRunReconciliation -Manifests @(
        (New-ReconRun -Nonce "debt-a" -Rows @((New-ReconRow -Violating @("dc0"))) `
                -Candidates @($debtCandidateA) -Constructs $dcConstructs -ConstructFiles $debtFiles),
        (New-ReconRun -Nonce "debt-b" -Rows @((New-ReconRow -Violating @("dc0"))) `
                -Candidates @($debtCandidateB) -Constructs $dcConstructs -ConstructFiles $debtFiles))
    Assert-Replay (@($debtAgreement.candidates).Count -eq 1 -and
        @($debtAgreement.candidates)[0].disposition -ceq "semanticAgreementTextWithheld" -and
        [string]@($debtAgreement.candidates)[0].semanticIdentity.remediation.existingDebtFollowUp.evidenceFactId -ceq
            $debtEvidenceId) `
        "Follow-up wording variants over identical bounded actions did not reconcile structurally."
    $debtBaseManifest = New-ReconRun -Nonce "identity-base" `
        -Rows @((New-ReconRow -Violating @("dc0"))) -Candidates @($debtCandidateA) `
        -Constructs $dcConstructs -ConstructFiles $debtFiles
    $debtBaseIdentity = Get-ReviewerRunReconciliationSemanticCandidateIdentity `
        -Candidate $debtCandidateA -Manifest $debtBaseManifest
    $debtActionVariant = $debtCandidateA | ConvertTo-Json -Depth 32 | ConvertFrom-Json -Depth 32
    $debtActionVariant.existingDebtFollowUp.action = "linkTrackedFollowUp"
    ...
    $debtEvidenceManifest = New-ReconRun -Nonce "identity-evidence" `
        ...
        -Constructs $dcConstructs -ConstructFiles $debtEvidenceFiles
    foreach ($variant in @(
            @{ Name = "action"; Candidate = $debtActionVariant; Manifest = $debtActionManifest },
            @{ Name = "evidence"; Candidate = $debtEvidenceVariant; Manifest = $debtEvidenceManifest })) {
        $variantIdentity = Get-ReviewerRunReconciliationSemanticCandidateIdentity `
            -Candidate $variant.Candidate -Manifest $variant.Manifest
        Assert-Replay ([bool]$debtBaseIdentity.valid -and [bool]$variantIdentity.valid -and
            (Get-ReviewerRunReconciliationSemanticCandidateBucketKey -Identity $debtBaseIdentity) -cne
            (Get-ReviewerRunReconciliationSemanticCandidateBucketKey -Identity $variantIdentity)) `
            "A near-collision with different debt $($variant.Name) reconciled as the same action. Base errors: $($debtBaseIdentity.errors -join '; '); variant errors: $($variantIdentity.errors -join '; ')"
    }
    $scopePayload = $debtBaseIdentity.payload | ConvertTo-Json -Depth 32 | ConvertFrom-Json -Depth 32
    $scopePayload.remediation.existingDebtFollowUp.scopePath = "/src/other.cs"
    $scopeCanonical = ConvertTo-ReviewerConventionSpecialistCanonicalJson -Value $scopePayload
    $scopeIdentity = [pscustomobject]@{ canonicalPayload = $scopeCanonical }
    Assert-Replay ((Get-ReviewerRunReconciliationSemanticCandidateBucketKey -Identity $debtBaseIdentity) -cne
        (Get-ReviewerRunReconciliationSemanticCandidateBucketKey -Identity $scopeIdentity)) `
        "A near-collision with a different bounded debt scope reconciled as the same action."
    ...
    Assert-SemanticCandidateSplit -Name "follow-up" `
        ...
        -RightCandidate (New-ReconCandidate -Path "src/semantic.cs" -Line 1034 -RemediationTargets dc0 `
            -ConventionKey AlternateRequiredConstruct)
    ...
    $missingFollowUp.PSObject.Properties.Remove("existingDebtFollowUp")
    ...
    $stringTypedIdentity.existingDebtFollowUp.comparableCount = "0"
    ...
    $badMetadataSchema.candidates[0].semanticIdentity.remediation.changedCodeFix.targets = @("dc0")
