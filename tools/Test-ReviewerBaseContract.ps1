...
try {
    ...
    Assert-Pass 'the superseded base identity sealed fixtures name is still accepted' {
        ...
        if ([string]$acceptance.mode -cne 'consolidated-equivalent-tree-integrated-carrier') { throw "mode was $($acceptance.mode)" }
        if ([string]$acceptance.boundaryCommit -cne ([string]$active.baseCommit)) { throw 'wrong boundary' }
        if ([string]$acceptance.integratedCarrierCommit -cne $script:ReviewerBaseContractIntegratedCarrierCommit) {
            throw 'wrong integrated carrier'
        }
    ...
    Assert-Pass 'the active base identity is accepted directly' {
        ...
        if ([string]$acceptance.mode -cne 'active-lineage-integrated-carrier') { throw "mode was $($acceptance.mode)" }
    }
    ...
    # The line-ending basis. git rewrites the bound artifacts' line endings per
    # checkout, so a raw-byte digest pinned the checkout rather than the content
    # and every LF checkout was refused. These checks prove the normalising
    # basis fixes that WITHOUT making a content change invisible.
    $eolRoot = Join-Path $sandbox 'eol'
    [void](New-Item -ItemType Directory -Path $eolRoot -Force)
    $sealedContract = Read-ReviewerBaseContract -Path (Get-ReviewerBaseContractDefaultPath -RepoRoot $RepoRoot)

    function Copy-BoundArtifactsWithLineEnding {
        param([Parameter(Mandatory)][string]$Destination, [Parameter(Mandatory)][string]$NewLine)
        if (Test-Path -LiteralPath $Destination) { Remove-Item -LiteralPath $Destination -Recurse -Force }
        [void](New-Item -ItemType Directory -Path $Destination -Force)
        foreach ($artifact in @($sealedContract.boundArtifacts)) {
            $relative = ([string]$artifact.path) -replace '/', [IO.Path]::DirectorySeparatorChar
            $target = Join-Path $Destination $relative
            [void](New-Item -ItemType Directory -Path (Split-Path -Parent $target) -Force)
            [string]$text = [IO.File]::ReadAllText((Join-Path $RepoRoot $relative), $script:Utf8)
            [string]$body = ($text -replace "`r`n", "`n") -replace "`r", "`n"
            if ($NewLine -cne "`n") { $body = $body -replace "`n", $NewLine }
            [IO.File]::WriteAllText($target, $body, $script:Utf8)
        }
    }

    Assert-Pass 'the same content digests identically whether it is checked out CRLF or LF' {
        $crlf = Join-Path $eolRoot 'sample-crlf.txt'
        $lf = Join-Path $eolRoot 'sample-lf.txt'
        [IO.File]::WriteAllText($crlf, "one`r`ntwo`r`n", $script:Utf8)
        [IO.File]::WriteAllText($lf, "one`ntwo`n", $script:Utf8)
        [string]$crlfText = Get-ReviewerBaseContractArtifactDigest -Path $crlf -Basis 'text-lf-v1'
        [string]$lfText = Get-ReviewerBaseContractArtifactDigest -Path $lf -Basis 'text-lf-v1'
        if ($crlfText -cne $lfText) { throw "the normalising basis still split the checkouts: $crlfText vs $lfText" }
        # And the record of WHY the basis had to change: raw bytes disagree.
        [string]$crlfRaw = Get-ReviewerBaseContractArtifactDigest -Path $crlf -Basis 'raw-bytes-v1'
        [string]$lfRaw = Get-ReviewerBaseContractArtifactDigest -Path $lf -Basis 'raw-bytes-v1'
        if ($crlfRaw -ceq $lfRaw) { throw 'raw bytes agreed across line endings, so this test proves nothing' }
    }

    Assert-Pass 'a lone carriage return normalises to one line feed, not two' {
        $cr = Join-Path $eolRoot 'sample-cr.txt'
        $lf = Join-Path $eolRoot 'sample-lf.txt'
        [IO.File]::WriteAllText($cr, "one`rtwo`r", $script:Utf8)
        [IO.File]::WriteAllText($lf, "one`ntwo`n", $script:Utf8)
        if ((Get-ReviewerBaseContractArtifactDigest -Path $cr -Basis 'text-lf-v1') -cne
            (Get-ReviewerBaseContractArtifactDigest -Path $lf -Basis 'text-lf-v1')) {
            throw 'a lone CR did not normalise to a single LF'
        }
    }

    Assert-Pass 'an LF checkout of every bound artifact satisfies the sealed contract' {
        Copy-BoundArtifactsWithLineEnding -Destination (Join-Path $eolRoot 'lf') -NewLine "`n"
        [string]$objection = Test-ReviewerBaseContractBoundArtifact -Contract $sealedContract `
            -RepoRoot (Join-Path $eolRoot 'lf')
        if ($objection.Length -gt 0) { throw $objection }
    }

    Assert-Pass 'a CRLF checkout of every bound artifact satisfies the same contract' {
        Copy-BoundArtifactsWithLineEnding -Destination (Join-Path $eolRoot 'crlf') -NewLine "`r`n"
        [string]$objection = Test-ReviewerBaseContractBoundArtifact -Contract $sealedContract `
            -RepoRoot (Join-Path $eolRoot 'crlf')
        if ($objection.Length -gt 0) { throw $objection }
    }

    Assert-Pass 'a content change inside an LF checkout is still refused' {
        $tampered = Join-Path $eolRoot 'tampered'
        Copy-BoundArtifactsWithLineEnding -Destination $tampered -NewLine "`n"
        $victim = Join-Path $tampered (([string]@($sealedContract.boundArtifacts)[0].path) -replace '/', [IO.Path]::DirectorySeparatorChar)
        [IO.File]::WriteAllText($victim, ([IO.File]::ReadAllText($victim, $script:Utf8) + "# tamper`n"), $script:Utf8)
        [string]$objection = Test-ReviewerBaseContractBoundArtifact -Contract $sealedContract -RepoRoot $tampered
        if ($objection.Length -eq 0) { throw 'a tampered artifact was accepted under the normalising basis' }
    }

    Assert-Pass 'an artifact carrying a NUL byte is refused rather than normalised' {
        $binary = Join-Path $eolRoot 'binary.bin'
        [IO.File]::WriteAllBytes($binary, [byte[]]@(1, 0, 13, 10, 2))
        $refused = $false
        try { Get-ReviewerBaseContractArtifactDigest -Path $binary -Basis 'text-lf-v1' | Out-Null }
        catch { $refused = $true }
        if (-not $refused) { throw 'a NUL-carrying file was normalised' }
    }

    Assert-Refused 'a contract declaring an unknown artifact hash basis is refused' 'cannot compute' {
        $mutated = Join-Path $sandbox 'unknown-basis.json'
        $copy = Get-Content -LiteralPath (Get-ReviewerBaseContractDefaultPath -RepoRoot $RepoRoot) -Raw |
            ConvertFrom-Json -Depth 32
        $copy.artifactHashBasis = 'text-utf16-v9'
        $copy.contractDigest = Get-ReviewerBaseContractDigest -Contract $copy
        [IO.File]::WriteAllText($mutated, ($copy | ConvertTo-Json -Depth 32 -Compress:$false), $script:Utf8)
        Read-ReviewerBaseContract -Path $mutated
    }
}
finally {
