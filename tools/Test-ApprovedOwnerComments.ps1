...
function New-Evidence {
    ...
    return [pscustomobject]@{
        ...
        Subject = [ordered]@{
            ...
            reviewer = [ordered]@{ operatorAlias = 'operator' }
        ...
        switch ($Action) {
            ...
            'ListThreads' {
                foreach ($thread in $script:ProviderThreads) {
                    if ($thread -is [Collections.IDictionary] -and -not $thread.Contains('threadContext')) {
                        $thread['threadContext'] = [ordered]@{
                            filePath = [string](Get-ApprovedOwnerValue $thread 'filePath' '/tests/WidgetTests.cs')
                            rightFileStart = [ordered]@{ line = [int](Get-ApprovedOwnerValue $thread 'line' 27) }
                        }
                    }
                    foreach ($comment in @(Get-ApprovedOwnerValue $thread 'comments' @())) {
                        if ($comment -is [Collections.IDictionary] -and -not $comment.Contains('author')) {
                            $comment['author'] = [ordered]@{ uniqueName = 'operator@contoso.example' }
                        }
                        if ($comment -is [Collections.IDictionary] -and -not $comment.Contains('commentType')) { $comment['commentType'] = 1 }
                        if ($comment -is [Collections.IDictionary] -and -not $comment.Contains('isDeleted')) { $comment['isDeleted'] = $false }
                    }
                }
                return $script:ProviderThreads.ToArray()
            }
            'CreateThread' {
...
try {
    ...
    Check 'valid double-dot filename is accepted without permitting traversal' `
        ((ConvertTo-ApprovedOwnerCanonicalPath '/tests/A..B.cs') -ceq '/tests/A..B.cs')
    ...
    [void]$script:ProviderThreads.Add([ordered]@{
            id = 71; status = 'active'
            threadContext = [ordered]@{ filePath = '/tests/WidgetTests.cs'; rightFileStart = [ordered]@{ line = 27 } }
            comments = @([ordered]@{ content = $preview.body; author = [ordered]@{ uniqueName = 'attacker@contoso.example' }; commentType = 1; isDeleted = $false })
        })
    Refuses 'foreign-author dedupe marker cannot suppress an approved finding' {
        Invoke-ApprovedOwnerComment $evidence @('dc0') @($finding) $provider 'foreign marker'
    } 'foreign author or anchor'
    Check 'foreign-author marker refusal performs zero provider writes' `
        (@($script:ProviderCalls | Where-Object { $_ -in @('CreateThread', 'UpdateStatus') }).Count -eq 0)

    $provider = New-FakeProvider
    [void]$script:ProviderThreads.Add([ordered]@{
            id = 72; status = 'active'
            threadContext = [ordered]@{ filePath = '/tests/Other.cs'; rightFileStart = [ordered]@{ line = 27 } }
            comments = @([ordered]@{ content = $preview.body; author = [ordered]@{ uniqueName = 'operator@contoso.example' }; commentType = 1; isDeleted = $false })
        })
    Refuses 'wrong-anchor dedupe marker cannot enroll an unrelated thread' {
        Invoke-ApprovedOwnerComment $evidence @('dc0') @($finding) $provider 'wrong anchor' -Publish -ApproveUpdate
    } 'foreign author or anchor'
    Check 'wrong-anchor marker refusal performs zero provider writes' `
        (@($script:ProviderCalls | Where-Object { $_ -in @('CreateThread', 'UpdateStatus') }).Count -eq 0)

    $sameNamedMethod = New-Evidence -ConstructId 'dc1' -Symbol 'Creates_widget'
    $sameNamedMethod.Coverage.changedConstructs[0].line = 41
    $sameNamedMethod.Coverage.changedConstructs[0].endLine = 41
    $sameNamedFinding = Get-ApprovedOwnerFindingId $sameNamedMethod.Subject.headKey rs0 dc1 `
        $sameNamedMethod.Coverage.changedConstructs[0].path $sameNamedMethod.Coverage.changedConstructs[0].name
    $sameNamedSelections = Resolve-ApprovedOwnerSelections $sameNamedMethod @('dc1') @($sameNamedFinding)
    $sameNamedSelection = @($sameNamedSelections)[0]
    Check 'same-named methods at different lines have distinct dedupe keys' `
        ($sameNamedSelection.dedupeKey -cne $preview.dedupeKey)

    $provider = New-FakeProvider
    foreach ($threadId in 1..(Get-ApprovedOwnerThreadPageSize)) {
        [void]$script:ProviderThreads.Add([ordered]@{
                id = $threadId; status = 'active'; comments = @([ordered]@{ content = 'unrelated' })
            })
    }
    Refuses 'a full bounded thread page is refused before dedupe or audit decisions' {
        Invoke-ApprovedOwnerComment $evidence @('dc0') @($finding) $provider 'bounded page'
    } 'incomplete'
    Check 'full thread page refusal performs zero provider writes' `
        (@($script:ProviderCalls | Where-Object { $_ -in @('CreateThread', 'UpdateStatus') }).Count -eq 0)

    $provider = New-FakeProvider
    foreach ($threadId in 1..((Get-ApprovedOwnerThreadPageSize) - 1)) {
        [void]$script:ProviderThreads.Add([ordered]@{
                id = $threadId; status = 'active'; comments = @([ordered]@{ content = 'unrelated' })
            })
    }
    Refuses 'thread-page capacity for post-write confirmation is reserved before mutation' {
        Invoke-ApprovedOwnerComment $evidence @('dc0') @($finding) $provider 'bounded capacity' -Publish
    } 'insufficient capacity'
    Check 'insufficient confirmation capacity performs zero provider writes' `
        (@($script:ProviderCalls | Where-Object { $_ -in @('CreateThread', 'UpdateStatus') }).Count -eq 0)
    $nearFullDryRun = Invoke-ApprovedOwnerComment $evidence @('dc0') @($finding) $provider 'near-full dry run'
    Check 'near-full thread page still permits a zero-write dry run' `
        ($nearFullDryRun.results[0].outcome -ceq 'wouldCreate' -and $nearFullDryRun.providerWrites -eq 0)
    $script:ProviderThreads[0].comments[0].content = $preview.body
    $nearFullNoOp = Invoke-ApprovedOwnerComment $evidence @('dc0') @($finding) $provider 'near-full no-op' -Publish
    Check 'near-full thread page still permits an exact zero-write no-op' `
        ($nearFullNoOp.results[0].outcome -ceq 'noOp' -and $nearFullNoOp.providerWrites -eq 0)

    $batchEvidence = New-Evidence
    $batchEvidence.Coverage.changedConstructs = @(
        $batchEvidence.Coverage.changedConstructs[0],
        [ordered]@{ constructId = 'dc1'; kind = 'declaration'; path = '/tests/WidgetTests.cs'
            line = 27; endLine = 27; name = 'Updates_widget'; attributes = @('TestMethod'); status = 'known' }
    )
    $batchEvidence.Status.violations = @(
        [ordered]@{ ruleRef = 'rs0'; constructRef = 'dc0' },
        [ordered]@{ ruleRef = 'rs0'; constructRef = 'dc1' }
    )
    $batchEvidence.Coverage.rows[0].violatingConstructs = @('dc0', 'dc1')
    $batchFinding2 = Get-ApprovedOwnerFindingId $batchEvidence.Subject.headKey rs0 dc1 '/tests/WidgetTests.cs' 'Updates_widget'
    $provider = New-FakeProvider
    foreach ($threadId in 1..((Get-ApprovedOwnerThreadPageSize) - 2)) {
        [void]$script:ProviderThreads.Add([ordered]@{
                id = $threadId; status = 'active'; comments = @([ordered]@{ content = 'unrelated' })
            })
    }
    Refuses 'multi-selection capacity is refused before the first partial write' {
        Invoke-ApprovedOwnerComment $batchEvidence @('dc0', 'dc1') @($finding, $batchFinding2) $provider 'batch capacity' -Publish
    } 'complete approved batch'
    Check 'multi-selection capacity refusal performs zero provider writes' `
        (@($script:ProviderCalls | Where-Object { $_ -in @('CreateThread', 'UpdateStatus') }).Count -eq 0)
    ...
    $provider = New-FakeProvider -FailCreate
    [void]$script:ProviderThreads.Add([ordered]@{ id = 81; status = 'active'; comments = @([ordered]@{
                    content = "different`n<!-- devpilot-owner-comment:v1:$($preview.dedupeKey) -->"
                }) })
    Refuses 'failed replacement create does not close the existing collision' {
        Invoke-ApprovedOwnerComment $evidence @('dc0') @($finding) $provider 'replacement fails' -Publish -ApproveUpdate
    } 'fake provider create failure'
    Check 'existing collision stays active when replacement creation fails' `
        ([string]$script:ProviderThreads[0].status -ceq 'active' -and
        @($script:ProviderCalls | Where-Object { $_ -ceq 'UpdateStatus' }).Count -eq 0)

    $provider = New-FakeProvider
    [void]$script:ProviderThreads.Add([ordered]@{ id = 82; status = 'active'; comments = @([ordered]@{ content = $preview.body }) })
    [void]$script:ProviderThreads.Add([ordered]@{ id = 83; status = 'active'; comments = @([ordered]@{
                    content = "different`n<!-- devpilot-owner-comment:v1:$($preview.dedupeKey) -->"
                }) })
    $repairedCollision = Invoke-ApprovedOwnerComment $evidence @('dc0') @($finding) $provider 'repair stale collision' -Publish -ApproveUpdate
    Check 'exact replacement plus stale collision repairs without creating a duplicate' `
        ($repairedCollision.results[0].outcome -ceq 'updated' -and $repairedCollision.providerWrites -eq 1 -and
        @($script:ProviderCalls | Where-Object { $_ -ceq 'CreateThread' }).Count -eq 0 -and
        [string]$script:ProviderThreads[0].status -ceq 'active' -and [string]$script:ProviderThreads[1].status -ceq 'fixed')
    ...
    $oldIntent = [ordered]@{
        ...
        headKey = $evidence.Subject.headKey; selections = @([ordered]@{
                ...
                path = $preview.path; line = $preview.line
            })
    }
    ...
    Check 'operator command requires explicit approval, commenter identity, and defaults dry-run' (
        $commandText -match '\[Parameter\(Mandatory\)\]\[switch\]\$Approve' -and
        $commandText -match '\[Parameter\(Mandatory\)\]\[ValidatePattern\([^\r\n]+\)\]\[string\]\$OperatorAlias' -and
        $commandText -match '-OperatorAlias \$OperatorAlias' -and
