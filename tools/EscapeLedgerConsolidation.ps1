...
function Resolve-EscapeLedgerConsolidatedCommit {
    ...
    $effectiveReplacement = $replacementCommitFull
    $effectiveBasis = [string]$entry.equivalenceBasis
    if (-not (Test-EscapeLedgerConsolidationAncestor -RepoRoot $RepoRoot -Ancestor $replacementCommitFull -Descendant $HeadRef)) {
        $carrierCommit = '80131f567a82997281bcdb754d0551795c59105d'
        $carrierTree = '7ac820bfc3e2857ae7f6933689a8eed67e64bf33'
        $actualCarrierTree = Get-EscapeLedgerConsolidationTree -RepoRoot $RepoRoot -Commit $carrierCommit
        if ($actualCarrierTree -cne $carrierTree -or
            -not (Test-EscapeLedgerConsolidationAncestor -RepoRoot $RepoRoot `
                -Ancestor $replacementCommitFull -Descendant $carrierCommit)) {
            return (& $refuse "replacement commit '$replacementCommitFull' is not reachable from $HeadRef")
        }
        $integrationProofPaths = @(
            'docs/escape-ledger-consolidation.v1.json',
            'docs/escape-ledger.v2.json',
            'tools/EscapeLedgerConsolidation.ps1',
            'tools/Test-EscapeLedger.ps1',
            'tools/Test-EscapeLedgerConsolidation.ps1',
            'tools/Test-PowerShellBoundaryHardening.ps1'
        )
        $headContent = Invoke-EscapeLedgerConsolidationGit -RepoRoot $RepoRoot `
            -Arguments (@('diff', '--quiet', $HeadRef, '--') + $integrationProofPaths)
        if ($headContent.ExitCode -ne 0) {
            return (& $refuse ("replacement commit '$replacementCommitFull' is not reachable from $HeadRef, and " +
                    'that revision does not carry the exact integration-proof files verified in this checkout'))
        }
        $headCommit = Invoke-EscapeLedgerConsolidationGit -RepoRoot $RepoRoot `
            -Arguments @('rev-parse', '--verify', '--quiet', "$HeadRef^{commit}")
        if ($headCommit.ExitCode -ne 0) {
            return (& $refuse "replacement commit '$replacementCommitFull' is not reachable from $HeadRef")
        }
        $effectiveReplacement = [string]$headCommit.Output
        $effectiveBasis += '+integrated-pr117-carrier'
    }
    ...
    return [pscustomobject]@{
        Accepted          = $true
        ReplacementCommit = $effectiveReplacement
        Reason            = $null
