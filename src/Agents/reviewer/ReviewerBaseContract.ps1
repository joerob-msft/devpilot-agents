...
# Exact read-only Owner preview carrier used by the final main-parent
# consolidation. The contract binds this verifier's hash, so changing either
# identity requires regenerating and reviewing the contract.
$script:ReviewerBaseContractIntegratedCarrierCommit = '80131f567a82997281bcdb754d0551795c59105d'
$script:ReviewerBaseContractIntegratedCarrierTree = '7ac820bfc3e2857ae7f6933689a8eed67e64bf33'
...
$script:ReviewerBaseContractTopLevelKeys = @(
    'artifactHashBasis', 'boundArtifacts', 'contractDigest', 'contractId', 'description', 'kind',
...
# How a bound artifact's bytes become the digest the contract records.
#
#   text-lf-v1  - every CRLF and every lone CR becomes a single LF before
#                 hashing. The bound artifacts are source text, and git rewrites
#                 their line endings per checkout: with core.autocrlf=true a
#                 Windows working tree holds CRLF while a Linux one holds LF. A
#                 raw-byte digest therefore pins the CHECKOUT rather than the
#                 CONTENT, so a contract sealed on Windows refused every LF
#                 checkout. Normalising first makes the digest a property of the
#                 content, which is what the contract is actually about.
#   raw-bytes-v1 - the bytes exactly as they sit on disk. Correct for content
#                 git does not rewrite, and kept as a named basis so binding
#                 such an artifact later does not require silently changing what
#                 an existing basis means.
#
# The basis is recorded IN the contract and covered by its digest, so it cannot
# be swapped after sealing, and an unknown basis is refused rather than guessed.
$script:ReviewerBaseContractArtifactHashBases = @('raw-bytes-v1', 'text-lf-v1')
$script:ReviewerBaseContractDefaultArtifactHashBasis = 'text-lf-v1'
...
function Read-ReviewerBaseContract {
    ...
    if (@($script:ReviewerBaseContractArtifactHashBases) -cnotcontains [string]$contract.artifactHashBasis) {
        throw ("The reviewer base lineage contract '$Path' declares artifact hash basis " +
            "'$([string]$contract.artifactHashBasis)', which this build cannot compute. A basis this " +
            'build does not implement is refused rather than guessed at.')
    }
...
function Get-ReviewerBaseContractArtifactDigest {
    <#
    .SYNOPSIS
        One bound artifact's digest under a named hash basis.
    .DESCRIPTION
        Under 'text-lf-v1' the bytes are line-ending normalised before hashing,
        so the same content digests identically in a CRLF checkout and an LF
        one. A file carrying a NUL byte is refused under that basis rather than
        normalised: NUL means the content is not the source text the basis was
        chosen for, and quietly rewriting bytes inside binary content would
        produce a digest that describes nothing on disk.
    #>
    param(
        [Parameter(Mandatory)][string]$Path,
        [Parameter(Mandatory)][string]$Basis
    )
    if (@($script:ReviewerBaseContractArtifactHashBases) -cnotcontains $Basis) {
        throw "The artifact hash basis '$Basis' is not one this build knows how to compute."
    }
    [byte[]]$bytes = [IO.File]::ReadAllBytes($Path)
    if ($Basis -ceq 'raw-bytes-v1') {
        return ([Convert]::ToHexString([Security.Cryptography.SHA256]::HashData($bytes))).ToLowerInvariant()
    }
    if ($bytes -contains 0) {
        throw ("The bound artifact '$Path' carries a NUL byte, so it cannot be hashed under the " +
            "line-ending-normalising basis 'text-lf-v1'.")
    }
    ...
    return ([Convert]::ToHexString(
            [Security.Cryptography.SHA256]::HashData($normalized.ToArray()))).ToLowerInvariant()
}
...
function Test-ReviewerBaseContractBoundArtifact {
    ...
    [string]$basis = [string]$Contract.artifactHashBasis
    foreach ($artifact in @($Contract.boundArtifacts)) {
        ...
        [string]$actual = ''
        try { $actual = Get-ReviewerBaseContractArtifactDigest -Path $full -Basis $basis }
        catch { return "The bound artifact '$([string]$artifact.path)' could not be hashed: $($_.Exception.Message)" }
...
function Assert-ReviewerBaseCommitAccepted {
    ...
    if ([int]$ancestry.ExitCode -ne 0) {
        $carrier = $script:ReviewerBaseContractIntegratedCarrierCommit
        [string]$carrierTree = Get-ReviewerBaseContractTree -RepoRoot $repoFull -Commit $carrier
        if ($carrierTree -cne $script:ReviewerBaseContractIntegratedCarrierTree) {
            throw ("The active reviewer base boundary '$boundary' is not an ancestor of '$HeadRef', and integrated " +
                "carrier '$carrier' is absent or names tree '$carrierTree' rather than the pinned carrier tree.")
        }
        $carrierAncestry = Invoke-ReviewerBaseContractGit -RepoRoot $repoFull `
            -ArgumentList @('merge-base', '--is-ancestor', $boundary, $carrier)
        if ([int]$carrierAncestry.ExitCode -ne 0) {
            throw ("The active reviewer base boundary '$boundary' is not an ancestor of '$HeadRef', and the pinned " +
                "integrated carrier '$carrier' does not descend from that boundary.")
        }
        $boundPaths = @($contract.boundArtifacts | ForEach-Object { [string]$_.path })
        $headContent = Invoke-ReviewerBaseContractGit -RepoRoot $repoFull `
            -ArgumentList (@('diff', '--quiet', $HeadRef, '--') + $boundPaths)
        if ([int]$headContent.ExitCode -ne 0) {
            throw ("The active reviewer base boundary '$boundary' is not an ancestor of '$HeadRef', and that revision " +
                'does not carry the exact contract-bound reviewer artifacts verified in this checkout.')
        }
        $mode += '-integrated-carrier'
    }
    ...
    return [pscustomobject][ordered]@{
        ...
        integratedCarrierCommit = $(if ($mode -like '*-integrated-carrier') {
                $script:ReviewerBaseContractIntegratedCarrierCommit
            } else { '' })
        integratedCarrierTree = $(if ($mode -like '*-integrated-carrier') {
                $script:ReviewerBaseContractIntegratedCarrierTree
            } else { '' })
        mode                 = [string]$mode
