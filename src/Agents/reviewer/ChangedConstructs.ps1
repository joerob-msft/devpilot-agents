...
function Get-ReviewerConstructSha256 {
    param([Parameter(Mandatory)][AllowEmptyString()][string]$Text)
    $sha = [Security.Cryptography.SHA256]::Create()
    try {
        return ([BitConverter]::ToString(
                $sha.ComputeHash($script:ReviewerConstructUtf8.GetBytes($Text)))).Replace("-", "").ToLowerInvariant()
    }
    finally { $sha.Dispose() }
}

function Test-ReviewerConstructGeneratedCode {
    param(
        [Parameter(Mandatory)][AllowEmptyString()][string]$Path,
        [AllowEmptyCollection()][string[]]$Lines = @()
    )
    $canonical = $Path.Replace('\', '/').ToLowerInvariant()
    if ($canonical -match '(^|/)(bin|obj|generated|autogen|generatedfiles)(/|$)|\.(g|generated|designer)\.[^.]+$') {
        return $true
    }
    $prefix = (@($Lines | Select-Object -First 12) -join "`n").ToLowerInvariant()
    return ($prefix -match '<auto-generated|@generated|code generated .+ do not edit|this file (?:was|is) generated|generated by .+ (?:compiler|generator|tool)')
}
...
function Get-ReviewerConstructAttributeFrequency {
    ...
    for ($index = 0; $index -lt $DeclarationIndex.Count; $index++) {
        ...
        foreach ($attribute in @($declaration.Attributes)) {
            if (-not $attributesOnDeclaration.Add([string]$attribute)) { continue }
...
function Get-ReviewerChangedConstructs {
    ...
    foreach ($file in $ordered) {
        ...
        [void]$fileSummaries.Add([pscustomobject][ordered]@{
                evidenceFactId = "rdf1:$(Get-ReviewerConstructSha256 -Text $evidenceText)"
                path = $path
