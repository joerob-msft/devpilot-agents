...
function Complete-OwnerObserverRefusalObservation {
    param([Parameter(Mandatory)][Collections.IDictionary]$Observation)

    $implementation = Get-OwnerObserverValue $Observation 'implementation'
    $lifecycle = Get-OwnerObserverValue $Observation 'lifecycle'
    $execution = Get-OwnerObserverValue $Observation 'execution'
    if ([string](Get-OwnerObserverValue $Observation 'kind') -cne 'owner-observation' -or
        [string](Get-OwnerObserverValue $implementation 'id') -cne 'owner-v2-preview-orchestrator' -or
        [string](Get-OwnerObserverValue $lifecycle 'status') -cne $script:OwnerObserverUnknown -or
        [string](Get-OwnerObserverValue $execution 'refusalReason') -cne 'orchestrator-refusal') {
        return
    }

    $defaulted = [Collections.Generic.List[string]]::new()
    if (-not $Observation.Contains('subject')) {
        $Observation['subject'] = [ordered]@{
            pullRequestId = $script:OwnerObserverUnknown
            repositoryId = $script:OwnerObserverUnknown
            headCommit = $script:OwnerObserverUnknown
            targetCommit = $script:OwnerObserverUnknown
            targetRef = $script:OwnerObserverUnknown
        }
        [void]$defaulted.Add('subject')
    }
    if (-not $Observation.Contains('rule')) {
        $Observation['rule'] = [ordered]@{
            identity = $script:OwnerObserverUnknown
            path = $script:OwnerObserverUnknown
            section = $script:OwnerObserverUnknown
            commit = $script:OwnerObserverUnknown
            sha256 = $script:OwnerObserverUnknown
        }
        [void]$defaulted.Add('rule')
    }
    if (-not $Observation.Contains('counts')) {
        $Observation['counts'] = [ordered]@{
            checked = $script:OwnerObserverUnknown
            violations = $script:OwnerObserverUnknown
            unknown = $script:OwnerObserverUnknown
            uncovered = $script:OwnerObserverUnknown
        }
        [void]$defaulted.Add('counts')
    }
    if (-not $Observation.Contains('findingsComplete')) {
        $Observation['findingsComplete'] = $script:OwnerObserverUnknown
        [void]$defaulted.Add('findingsComplete')
    }
    if (-not $Observation.Contains('findings')) {
        $Observation['findings'] = @()
        [void]$defaulted.Add('findings')
    }

    $effects = Get-OwnerObserverValue $Observation 'effects'
    if ($effects -is [Collections.IDictionary]) {
        if (-not $effects.Contains('dedupe')) {
            $effects['dedupe'] = [ordered]@{
                created = $script:OwnerObserverUnknown
                updated = $script:OwnerObserverUnknown
                noOp = $script:OwnerObserverUnknown
                wouldCreate = $script:OwnerObserverUnknown
            ...
            [void]$defaulted.Add('effects.dedupe')
        }
        if (-not $effects.Contains('operatorIntervention')) {
            $effects['operatorIntervention'] = $script:OwnerObserverUnknown
            [void]$defaulted.Add('effects.operatorIntervention')
        }
    }
    if (-not $Observation.Contains('sourceArtifacts')) {
        $Observation['sourceArtifacts'] = @()
        [void]$defaulted.Add('sourceArtifacts')
    }

    if ($defaulted.Count -gt 0) {
        $errors = [Collections.Generic.List[string]]::new()
        foreach ($error in @(Get-OwnerObserverValue $Observation 'validationErrors' @())) {
            Add-OwnerObserverValidationError -Errors $errors -Text ([string]$error)
        }
        Add-OwnerObserverValidationError -Errors $errors -Text (
            'Owner v2 refusal observation omitted comparison fields defaulted to unknown: ' +
            ($defaulted -join ', ') + '.')
        $Observation['validationErrors'] = @($errors.ToArray())
    }
}
...
function ConvertFrom-OwnerNormalizedObservationBytes {
    ...
    Complete-OwnerObserverRefusalObservation -Observation $outcome
    $outcome = ConvertTo-OwnerObservationV2 -Observation $outcome
    if (-not (Test-OwnerObservation -Observation $outcome)) {
