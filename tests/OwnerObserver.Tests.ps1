...
Describe 'Owner observer normalized contract' {
    ...
    It 'completes caught Owner v2 refusal artifacts with explicit unknowns' {
        $partial = [ordered]@{
            schemaVersion = 1
            ...
            execution = [ordered]@{
                ...
                modelStarts = 0
                latencyMs = 0
                refusalReason = 'orchestrator-refusal'
                incompleteReason = 'orchestrator-refusal'
            }
            effects = [ordered]@{
                providerWrites = 0
                writeToolInvocations = 0
            }
            validationErrors = @('Evidence was unavailable.')
        }
        $path = Join-Path $TestDrive 'partial-v2-refusal.json'
        Write-OwnerObserverTestText -Path $path -Text ($partial | ConvertTo-Json -Depth 64)

        $outcome = Read-OwnerNormalizedObservation -Path $path

        $outcome.lifecycle.completed | Should -Be 'unknown'
        $outcome.subject.pullRequestId | Should -Be 'unknown'
        $outcome.rule.identity | Should -Be 'unknown'
        $outcome.counts.checked | Should -Be 'unknown'
        $outcome.findingsComplete | Should -Be 'unknown'
        $outcome.findings | Should -HaveCount 0
        $outcome.effects.dedupe.unknown | Should -Be 'unknown'
        $outcome.effects.operatorIntervention | Should -Be 'unknown'
        $outcome.validationErrors | Should -Contain 'Evidence was unavailable.'
        $outcome.validationErrors[-1] | Should -Match 'defaulted to unknown'
        Test-OwnerObservation -Observation $outcome | Should -BeTrue
    }
