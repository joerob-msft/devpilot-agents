...
Describe 'Owner parity snapshots, paths, and sanitization' {
    ...
    It 'preserves an unknown run state when the candidate observation is unavailable' {
        $manifestPath = Join-Path $TestDrive 'unknown-run.json'
        Copy-Item -LiteralPath (
            Join-Path $script:RepoRoot 'tests/fixtures/owner-orchestrator/generic-cohort.json'
        ) -Destination $manifestPath
        $stateRoot = Join-Path $TestDrive 'unknown-run-state'
        $baseline = New-TestParityObservation

        InModuleScope DevPilot.OwnerParity -Parameters @{
            ManifestPath = $manifestPath
            StateRoot = $stateRoot
            Baseline = $baseline
        } {
            param($ManifestPath, $StateRoot, $Baseline)
            $identity = 'owner-v2:test-unknown-candidate'
            $capabilityRoot = Join-Path $StateRoot 'capability'
            [void](New-Item -ItemType Directory -Path $capabilityRoot -Force)
            Mock Invoke-OwnerV2PreviewPrepare {
                [pscustomobject]@{
            ...
            $candidate = Resolve-OwnerParityCandidate -Candidate ([ordered]@{
                    mode = 'run'
                    manifestPath = $ManifestPath
                    entryId = 'generic-replay-owner-cohort'
                }) -V2StateRoot $StateRoot
            $evaluation = New-OwnerParityUnavailableEvaluation `
                -Baseline $Baseline `
                -Adjudication @() `
                -Reason $candidate.blockingReason `
                -CandidateCompleted $candidate.candidateCompleted
            $gate = New-OwnerParityCompletionGate -EntryReports @(
                [pscustomobject]@{ metrics = $evaluation.metrics }
            )

            $candidate.runReason | Should -Be 'orchestrator-refusal'
